...
class MysqlData:
    ...
    def query_page(self, field=None, page='PageMax', fields=None):
        """
        ...
        """
        if fields:
            sql = f'SELECT MAX({page}) FROM {self.table} WHERE {fields} = "{field}";'
            ...
            sql = f"SELECT MAX({page}) FROM {self.table};"
            try:
                # 执行查询语句
                self.curr.execute(sql)
                # result = self.curr.fetchall()
