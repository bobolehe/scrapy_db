...
from fortiguardPro.toolclass.OperateDB import MysqlData
from fortiguardPro.toolclass.proxy_module import GetProxy
...
class DataPipeline:
    ...
    def open_spider(self, spider):
        ...
        logger.error(self.data.field_exists(fields=vars(FortiguardproItem)['fields']))
        spider.data_max = 0
        spider.data = self.data
        spider.max = 0

    def process_item(self, item, spider):
        fields = vars(FortiguardproItem)['fields']
        for field in fields:
            if field == 'url':
                continue
            elif "time" in field:
