...
from endoflifePro.toolclass.OperateDB import MysqlData
from endoflifePro.toolclass.proxy_module import GetProxy
...
class DataPipeline:
    ...
    def open_spider(self, spider):
        ...
        logger.error(self.data.field_exists(fields=vars(EndoflifeproItem)['fields']))
        spider.data_max = 0
        spider.data = self.data

    def process_item(self, item, spider):
        fields = vars(EndoflifeproItem)['fields']
        for field in fields:
            if field == 'url':
                continue
            elif "time" in field:
