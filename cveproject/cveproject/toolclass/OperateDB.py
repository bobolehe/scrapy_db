...
class MysqlData:
    ...
    def disconnect(self, time="stime", table="table"):
        """
        关闭提供的链接以及游标
        :return: 返回提示信息
        """
        try:
            sql = f"SELECT id FROM {table} WHERE {time} >= '{self.ks_time}';"
    ...
    def create_fields(self, field=str(), fields=list(), table="biaoming"):
        """
        实现数据表字段的添加
        :return:
        """
    ...
    def field_exists(self, fields=dict(), table='biaoming'):
        """
    ...
    def query_page(self, field=None, name=None, fields=None, table='table'):
        """
        增量式更新功能情况
        :param name: 指定页数字段名称
